...
#----------------------------------------------------------------------------------------------
        ...
        print("To delete a client for a specific case, enter 18.")
        ...
        if k == 0:
            ...
            # cursor.execute("DELETE FROM ClientsCases WHERE ClientCase_ID = 2")
            # conn.commit()
            # ClientCase_ID = input("Enter the ClientCase_ID you want to delete: ")
            # cursor.execute("begin;") 
            # try:

            #     cursor.execute("DELETE FROM ClientsCases WHERE ClientCase_ID = %s", (ClientCase_ID,))
            #     conn.commit()
            #     print("client  details deleted  successfully from case.")
            # except psycopg2.Error as e:
            #     conn.rollback()
            #     print("Error deleting client details from case.:", e)


            ClientCase_ID = input("Enter the ClientCase_ID you want to delete: ")
            cursor.execute("begin;") 

            cursor.execute("SELECT 1 FROM ClientsCases WHERE ClientCase_ID = %s", (ClientCase_ID,))
            if cursor.fetchone():
                try:
                    cursor.execute("DELETE FROM ClientsCases WHERE ClientCase_ID = %s", (ClientCase_ID,))
                    conn.commit()
                    print("Client details for ClientCase_ID", ClientCase_ID, "deleted successfully from the case.")
                except psycopg2.Error as e:
                    conn.rollback()
                    print("Error deleting client details from the case:", e)
            else:
                conn.rollback()
                print("ClientCase_ID", ClientCase_ID, "does not exist in the table.")

            
        else:
            print("Choose number between 0 to 18.")
